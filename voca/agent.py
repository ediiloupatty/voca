...
def _tampilkan_menu_model(client) -> OpenAI:
    """Menu /model dua tahap: pilih provider, lalu pilih model (daftar live).

    - Provider belum ber-key -> langsung buka alur key + tes koneksi onboarding.
    - Daftar model diambil live dari endpoint /models provider (fallback kurasi
      bila fetch gagal), jadi tidak pernah usang.
    - Pilihan disimpan ke .env supaya bertahan antar sesi.
    """
    ...
    if pilihan is None:
        return client
    kode = opsi_menu[pilihan]["code"]

    # Provider belum ber-key? Tawarkan setup di tempat (ala wizard).
    if not provider.has_key(kode):
        if not onboarding.setup_provider(kode):
            return client

    # Tahap 2: pilih model — daftar diambil live.
    with console.status(f"[muted]mengambil daftar model {provider.name_of(kode)}…[/]",
                        spinner="dots", spinner_style="accent"):
        models, live = provider.list_models(kode)
    if not live:
        ui.info("  daftar live tak tersedia — menampilkan daftar bawaan.")

    model_kini = provider._PROVIDERS[kode]["model"]
    idx = models.index(model_kini) if model_kini in models else 0
    pilih_model = ui.pilih_item(f"PILIH MODEL — {provider.name_of(kode)}", models, idx)
    if pilih_model is None:
        return client

    onboarding.simpan_model(kode, models[pilih_model])
...
def main():
    # First-run setup ala Claude Code: kalau belum ada API key sama sekali,
    # wizard onboarding muncul (pilih provider → key → tes → simpan .env).
    # Jalur manual tetap ada: .env / env var terisi -> wizard tak muncul.
    from . import onboarding
    onboarding.pastikan_siap()
