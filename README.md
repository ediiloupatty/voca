...
- **Otak** — Qwen (`qwen-plus`) via DashScope (endpoint OpenAI-compatible) sebagai
  default. Provider lain tinggal pilih lewat `/model`: OpenAI, OpenRouter
  (Claude, GPT, Gemini, Grok, Llama — 300+ model dengan 1 key), DeepSeek,
  Gemini, Grok (xAI), Groq, Mistral, dan Ollama (lokal/offline). Daftar model
  diambil **live** dari endpoint tiap provider, jadi tidak pernah usang.
...
model suara, dan pasang perintah `voca`. Installer **tidak** menanyakan apa pun —
API key diminta lewat **wizard setup saat pertama kali menjalankan `voca`**
(pilih provider → masukkan key → tes koneksi), mirip first-run Claude Code.
...
# 3. API key — dua cara (pilih salah satu):
#    a) lewati langkah ini — wizard setup muncul saat pertama kali `voca` jalan
#    b) manual: cp .env.example .env, lalu isi DASHSCOPE_API_KEY di .env
